...
import { useLeads, deleteLead, updateLeadStatus } from "../hooks/useLeads";
...
export default function Leads() {
  ...
  const handleAddLead = async (lead: Partial<Lead>) => {
    const { addLead } = await import("../hooks/useLeads");
    await addLead(lead);
    refetch();
  };
      ...
      {!loading && filtered.length === 0 && (
        <div className="glass rounded-xl p-16 text-center">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {leads.length === 0 ? "Add your first lead or use Discover to find prospects." : "Try adjusting your search or filters."}
          </p>
          {leads.length === 0 && (
            <button
              onClick={() => setShowAddModal(true)}
