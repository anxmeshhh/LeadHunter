...
function normalisePlaceFields(p: any): {
  ...
  const rawPhone = p.nationalPhoneNumber ?? p.national_phone_number ?? p.phoneNumber ?? p.phone ?? p.internationalPhoneNumber ?? p.international_phone_number ?? "";
  const phone: string = typeof rawPhone === "string" ? rawPhone.trim() : "";
  const review_count: number = p.userRatingCount ?? p.user_rating_count ?? p.reviewCount ?? p.review_count ?? 0;
...
function computeOpportunity(lead: {
  ...
  if (!lead.phone) { ops.push("❌ No phone on Google — pitch GMB optimisation"); score += 10; }

  if (lead.review_count === 0)       { ops.push("📉 Zero Google reviews — pitch presence setup");      score += 20; }
  else if (lead.review_count < 10)   { ops.push("📉 Very few reviews — pitch review management");      score += 12; }
  else if (lead.review_count < 25)   { ops.push("📊 Below-average reviews — room to grow");            score += 5;  }

  if (lead.rating > 0 && lead.rating < 3.0)              { ops.push("⚠️ Poor rating (<3★) — pitch reputation management"); score += 20; }
  else if (lead.rating >= 3.0 && lead.rating < 3.8)      { ops.push("⚠️ Below-average rating — pitch review strategy");    score += 10; }

  if (lead.rawTypes.some((t) => HIGH_VALUE_TYPES.includes(t))) {
  ...
  const opportunityLabel: DiscoveredLead["opportunityLabel"] =
    score >= 55 ? "🔴 Hot Target" : score >= 22 ? "🟡 Warm Target" : "🟢 Low Priority";
...
async function generateAIPitch(lead: DiscoveredLead): Promise<string> {
  ...
  const websiteContext = lead.website ? `They have a website: ${lead.website}` : "They have NO website at all.";
...
async function fetchPlaces(
  ...
  const body: Record<string, unknown> = { textQuery: query, languageCode: "en", regionCode: "IN", pageSize: 20 };
...
export default function Discover() {
  ...
  // ── Save Lead — NOW WITH user_id ─────────────────────────────────────────────
  async function handleSaveLead(placeId: string) {
    const lead = results.find((r) => r.place_id === placeId);
    if (!lead || lead.saved || lead.saving || !user?.id) return;
    ...
    try {
      ...
      setResults((prev) => prev.map((r) => r.place_id === placeId ? { ...r, saved: true, saving: false } : r));
    } catch (e) {
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
        ...
        <div className="flex gap-3 flex-wrap">
            ...
            <AnimatePresence>
              {showCityDD && (
                <motion.div initial={{ opacity: 0, y: -4, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                  ...
                  <button onClick={() => { setCity(""); setShowCityDD(false); }}
                  ...
                  {CITIES.map((c) => (
                    <button key={c} onClick={() => { setCity(c); setShowCityDD(false); }}
          ...
          <button onClick={handleSearch}
            ...
            {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Hunting...</> : <><Target className="w-4 h-4" /> Hunt Leads</>}
          </button>
        </div>
              ...
              return (
                <button key={cat.label}
                  ...
                  className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border text-center transition-all duration-200 group ${
                    active ? "border-primary bg-primary/10 scale-[1.04] shadow-lg shadow-primary/10"
              ...
              <div className="flex flex-wrap gap-2">
                {([
                  ...
                  <button key={f.key} onClick={() => setFilterMode(f.key)}
                    className={`px-3 py-1 rounded-lg text-[11px] font-stats border transition-all flex items-center gap-1.5 ${filterMode === f.key ? f.active : f.inactive}`}>
                    {f.label}
                    <span className={`text-[10px] font-bold px-1 py-0.5 rounded ${filterMode === f.key ? "bg-white/10" : "opacity-50"}`}>{f.count}</span>
                  </button>
                ))}
              </div>
            </div>
              ...
              {displayResults.map((lead, i) => {
                ...
                return (
                  <motion.div key={lead.place_id} layout
                    ...
                    className={`glass rounded-2xl overflow-hidden group transition-all duration-200 ${lead.saved ? "border-l-[3px] border-l-emerald-500/60" : s.card}`}
                    ...
                    <div className="px-4 pt-3 pb-2 flex items-center justify-between">
                      {lead.saved ? (
                        ...
                        <span className={`text-[10px] font-stats px-2 py-0.5 rounded-md border ${s.badge}`}>{lead.opportunityLabel}</span>
                      )}
                      <span className="text-[10px] font-stats text-muted-foreground/40 truncate ml-2 max-w-[120px]">{lead.category}</span>
                    </div>

                    <div className="px-4 pb-4 space-y-3">
                      <div>
                        <h3 className="text-[15px] font-heading font-bold text-foreground leading-snug line-clamp-1">{lead.business_name}</h3>
                      ...
                      {lead.rating > 0 && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] font-stats text-muted-foreground/50">({lead.review_count.toLocaleString()} reviews)</span>
                        </div>
                      )}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="h-1.5 rounded-full bg-muted/40 overflow-hidden">
                          <motion.div initial={{ width: 0 }} animate={{ width: `${lead.opportunityScore}%` }}
                      ...
                      {lead.aiPitch && expandedPitch === lead.place_id ? (
                        ...
                        <button onClick={() => lead.aiPitch ? setExpandedPitch(lead.place_id) : handleGeneratePitch(lead.place_id)}
                          ...
                          {lead.loadingPitch ? <><Loader2 className="w-3 h-3 animate-spin" /> Generating...</>
                            : lead.aiPitch   ? <><Sparkles className="w-3 h-3" /> Show Pitch</>
                            :                  <><Brain className="w-3 h-3 group-hover:scale-110 transition-transform" /> Generate AI Pitch</>}
                        </button>
                      )}

                      <button onClick={() => handleSaveLead(lead.place_id)} disabled={lead.saved || lead.saving}
      ...
      {!searched && !loading && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground font-stats mt-1">Pick a category or type a search — every lead is scored from real data</p>
          </div>
          <div className="flex flex-wrap justify-center gap-3">
            {QUICK_STARTS.map((cat) => (
              <button key={cat.label} onClick={() => { setSelectedCat(cat); setCustomQuery(""); }}
      ...
      <AnimatePresence>
        {unsavedVisible > 0 && searched && !loading && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
            ...
            style={{ background: "rgba(10,12,20,0.94)", border: "1px solid rgba(255,255,255,0.1)", backdropFilter: "blur(20px)", boxShadow: "0 8px 40px rgba(0,0,0,0.6)" }}>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
              <span className="text-sm font-heading font-semibold text-foreground">
                {unsavedVisible} lead{unsavedVisible !== 1 ? "s" : ""} ready to save
              </span>
              {filterMode !== "all" && <span className="text-[10px] font-stats text-muted-foreground">(filtered view)</span>}
            </div>
            <button onClick={handleSaveAll} disabled={savingAll}
              ...
              style={{ background: "var(--gradient-primary)" }}>
              {savingAll ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Saving...</> : <><Plus className="w-3.5 h-3.5" /> Save All to CRM</>}
            </button>
          </motion.div>
        )}
      </AnimatePresence>
