...
// Google Places type keys that indicate high-budget clients
...
function extractCity(address: string): string {
  ...
  // Indian addresses end with: ..., City, State PIN, India
  // Find "India" and step back 2 to get City
  const indiaIdx = parts.findIndex((p) => /^india$/i.test(p));
  if (indiaIdx >= 2) return parts[indiaIdx - 2];
  if (indiaIdx === 1) return parts[0];
  // Fallback: third from end usually = city
...
// ── Normalise raw API place object ─────────────────────────────────────────────
// RapidAPI proxy sometimes renames Google's camelCase keys.
// We read every known variant so a proxy rename never silently breaks detection.
...
// ── Opportunity Scoring ────────────────────────────────────────────────────────
// Every point is grounded in real Google Places data — nothing fabricated.
function computeOpportunity(lead: {
  ...
  // 2. Phone — missing = incomplete GMB listing
  if (!lead.phone) {
    ops.push("❌ No phone on Google — pitch GMB optimisation");
    score += 10;
  }

  // 3. Review volume — real visibility proxy
  ...
  // 6. No gaps = established — still worth pitching advanced services
  if (ops.length === 0) {
    ops.push("✅ Established presence — pitch SEO, ads or CRM");
  }
...
async function fetchPlaces(
  ...
  if (!apiKey || !apiHost) {
    throw new Error("Missing VITE_RAPIDAPI_KEY or VITE_RAPIDAPI_HOST in .env");
  }
  ...
  // Only add locationRestriction when no city — lets the query text handle city targeting
  // Rectangle covers entire Indian subcontinent (no radius limit issue unlike circle)
  if (!city) {
    body.locationRestriction = {
      rectangle: {
        low:  { latitude: 6.4627,  longitude: 68.1097 }, // SW India
        high: { latitude: 35.5133, longitude: 97.3953 }, // NE India
      },
    };
  }
  ...
  const res = await fetch(`https://${apiHost}/v1/places:searchText`, {
    ...
    headers: {
      ...
      // nextPageToken MUST be in FieldMask or pagination never works
      "X-Goog-FieldMask": [
        "places.id",
        "places.displayName",
  ...
  // DEV: log first raw result once so you can verify proxy field names
  ...
  const leads: DiscoveredLead[] = places.map((p) => {
    ...
    const base = {
      ...
      review_count,
      category:      extractCategory(rawTypes),
      rawTypes,
      saved:         false,
      saving:        false,
      aiPitch:       "",
    ...
    const scored = computeOpportunity(base);
    return { ...base, ...scored };
  });
...
export default function Discover() {
  ...
  async function handleGeneratePitch(placeId: string) {
    // Snapshot lead before async work to avoid stale closure
    const lead = results.find((r) => r.place_id === placeId);
    if (!lead || lead.loadingPitch) return;
    setResults((prev) => prev.map((r) =>
      r.place_id === placeId ? { ...r, loadingPitch: true } : r
    ));
    const pitch = await generateAIPitch(lead);
    setResults((prev) => prev.map((r) =>
      r.place_id === placeId ? { ...r, aiPitch: pitch, loadingPitch: false } : r
    ));
  ...
  async function handleSaveLead(placeId: string) {
    ...
    setResults((prev) => prev.map((r) =>
      r.place_id === placeId ? { ...r, saving: true } : r
    ));
  ...
  async function handleSaveAll() {
    // Snapshot IDs before any awaits — displayResults would be stale inside the loop
    const unsavedIds = displayResults
      .filter((r) => !r.saved && !r.saving)
      .map((r) => r.place_id);
            ...
            {HUNT_CATEGORIES.map((cat) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <span className="text-[8px] font-stats text-primary/70 leading-tight">{cat.reason}</span>
                  )}
                </button>
              );
            })}
                    ...
                    <div className="px-4 pb-4 space-y-3">
                      ...
                      <div className="space-y-1 pt-1 border-t border-white/[0.04]">
                        {lead.opportunities.map((opp, idx) => (
                          <p key={idx} className="text-[10px] font-stats text-muted-foreground/75 leading-relaxed">
                            {opp}
                          </p>
                        ))}
                      </div>
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center justify-between">
                          <span className="text-[9px] font-stats text-muted-foreground/40 uppercase tracking-widest">
                            Opportunity Score
                          </span>
                          <span className="text-[11px] font-stats font-bold text-foreground">
                      ...
                      {lead.aiPitch && expandedPitch === lead.place_id ? (
                        ...
                        <button
                          onClick={() =>
                            lead.aiPitch
                              ? setExpandedPitch(lead.place_id)
                              : handleGeneratePitch(lead.place_id)
                          }
