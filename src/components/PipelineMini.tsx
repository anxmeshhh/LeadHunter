...
export default function PipelineMini() {
  ...
  const fetchPipeline = useCallback(async () => {
    ...
    try {
      ...
      const built: PipelineStage[] = Object.entries(STAGE_CONFIG)
        ...
        .filter(([name]) => grouped[name]?.count > 0)
        .map(([name, cfg]) => ({
          name,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("realtime:pipeline-mini")
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-stats text-success">{formatValue(totalClosed)} closed</span>
        </div>
      </div>
        ...
        <div className="space-y-3">
          {stages.map((stage, i) => (
            <div key={stage.name} className="flex items-center gap-3">
              <span className="text-xs text-muted-foreground w-24 truncate font-stats">{stage.name}</span>
              <div className="flex-1 h-6 bg-muted rounded-md overflow-hidden">
                ...
                >
                  {stage.count > 0 && (
                    <span className="text-[10px] font-stats font-bold text-foreground">{stage.count}</span>
                  )}
                </motion.div>
              </div>
