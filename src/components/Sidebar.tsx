...
import { supabase } from "../lib/supabase"; // ✅ FIXED: use shared client
...
const navItems = [
  ...
  { to: "/followup",  icon: Bell,            label: "Follow-up Queue",color: "text-red-400",  highlight: true },
    ...
    >
      ...
      <div className="relative space-y-2.5">
        <div className="flex items-center justify-between">
          <span className="text-[9px] font-stats text-muted-foreground uppercase tracking-widest">Today's Goal</span>
          {streak > 0 && (
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: targetRow } = await supabase
        .from("daily_targets").select("followup_reminder_days")
      ...
      const { data: recentOutreach } = await supabase
        .from("outreach_history").select("lead_id")
        .eq("user_id", user.id)
        .gte("contacted_at", cutoff);

      const recentLeadIds = new Set((recentOutreach ?? []).map((o) => o.lead_id));
      const needsContact  = leads.filter((l) => !recentLeadIds.has(l.id)).length;
      setCount(needsContact);
    })();
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="px-3 py-2 shrink-0"
        ...
        <div className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg transition-all ${collapsed ? "justify-center" : ""}`}
      ...
      <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto
        ...
        {navItems.map((item) => {
          ...
          const isFollowUp = item.to === "/followup";
          const showBadge  = isFollowUp && followUpCount > 0;
            ...
            <NavLink key={item.to} to={item.to} className="block">
              ...
              >
                ...
                <div className="relative shrink-0">
                  ...
                  {/* Badge */}
                  {showBadge && (
                    <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-red-500 text-white text-[9px] font-stats font-bold flex items-center justify-center">
                ...
                <AnimatePresence>
                  {!collapsed && (
                    <motion.span
                      ...
                      className={`text-[13px] font-medium whitespace-nowrap transition-colors flex-1 ${
                        isActive ? "text-white" : item.highlight ? "text-red-400" : "text-slate-400 group-hover:text-slate-200"
                      }`}
                    >
                      {item.label}
                    </motion.span>
                ...
                {!collapsed && showBadge && (
                  <span className="ml-auto px-1.5 py-0.5 rounded-full bg-red-500 text-white text-[9px] font-stats font-bold">
                    {followUpCount > 9 ? "9+" : followUpCount}
                  </span>
                )}
      ...
      <div className="px-2 pb-2 shrink-0 space-y-1"
          ...
          <AnimatePresence>
            {!collapsed && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex-1 min-w-0">
                <p className="text-[11px] font-stats text-foreground truncate">{userEmail ?? "Loading..."}</p>
        ...
        <button onClick={handleSignOut} disabled={signingOut}
          className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all group disabled:opacity-50 ${collapsed ? "justify-center" : ""}`}
        ...
        >
          ...
          {collapsed && (
            <div className="absolute left-14 px-2.5 py-1.5 rounded-lg text-xs font-stats text-red-400
              pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity z-50 whitespace-nowrap"
              style={{ background: "rgba(10,12,20,0.97)", border: "1px solid rgba(248,113,113,0.2)" }}>
              Sign Out
            </div>
          )}
        </button>
